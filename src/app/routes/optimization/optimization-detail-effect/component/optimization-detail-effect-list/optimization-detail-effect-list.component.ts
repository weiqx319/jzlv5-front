...
@Component({
  selector: 'app-optimization-detail-effect-list',
  templateUrl: './optimization-detail-effect-list.component.html',
  styleUrls: ['./optimization-detail-effect-list.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class OptimizationDetailEffectListComponent implements OnInit {
  ...
  public viewTableData = {
    ...
    'summary_type': 'optimization_detail_effect',
    'selected_items': [],
  ...
  delOptimizationEffectDetails() {
    if (this.selected.length < 1) {
      this._message.error('请选择计划');
      return;
    }
    const postBody = { 'effect_details': { select_type: this.selectedType, details: [], sheets_setting: {}, type: 'campaign' } };
    if (this.selectedType === 'current' || this.pageInfo.allCount <= this.pageInfo.pageSize) {
      postBody['effect_details'].details = this.selected;
      postBody['effect_details'].select_type = 'current';
    } else {
      postBody['effect_details'].sheets_setting = { table_setting: { ...this.viewTableData, filter_optimization_ids: [this.optimizationId] } };
      postBody['effect_details'].select_type = 'all';
    }
    this.effectDetailService.delDetails(this.optimizationId, postBody).subscribe((result: any) => {
