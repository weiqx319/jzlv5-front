...
@Component({
  selector: 'app-optimization-detail-list',
  templateUrl: './optimization-detail-list.component.html',
  styleUrls: ['./optimization-detail-list.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [TableItemService]
})
export class OptimizationDetailListComponent implements OnInit, OnDestroy {
  public can_nav = true;
  ...
  public viewTableData = {
    ...
    summary_type: 'optimization_detail_ranking',
    selected_items: [],
  ...
  constructor(
    ...
    private rankingDetailService: OptimizationDetailRankingService,
    ...
    this.allFilterOption = this.optimizationItemService.getItemFilterType(
  ...
  ngOnInit() {
    ...
    this.viewTableData[
      'selected_items'
    ] = this.optimizationItemService.getDefaultItemsBySummaryType(
      this.viewTableData['summary_type']
    );
    this.viewTableData['sort_item'] = { key: 'pub_cost', dir: 'desc' };
    this.viewTableData[
      'locked_items'
    ] = this.optimizationItemService.getLockedItemsBySummaryType(
      this.viewTableData['summary_type']
    );
    ...
    if (localMarkInfo === null) {
      ...
      this.optimizationService.getCanJump().subscribe(result => {
        this.can_nav = result;
      });
    }
    // --
    ...
    //获取优化组信息
    this.optimizationService.getOptimizationInfo().subscribe(result => {
      if (result) {
  ...
  jumpToEdit(source = 'edit', count?) {
    ...
    this.editParameter.allViewTableData = {
      ...this.viewTableData,
      filter_optimization_ids: [this.optimizationId]
    };
  ...
  refreshCount() {
    if (!this.overdue) {
      ...
      // this.optimizationService.setOverdue(true);
  ...
  createBookMark($event) {
    const new_modal = this.modalService.create({
      ...
      nzComponentParams: {
        ...
        relationId: this.optimizationId
      }
    });
  ...
  delOptimizationRankingDetails() {
    ...
    if (
      ...
      postBody['ranking_details'].details = this.selected;
      postBody['ranking_details'].select_type = 'current';
    } else {
      postBody['ranking_details'].sheets_setting = {
        table_setting: {
          ...this.viewTableData,
          filter_optimization_ids: [this.optimizationId]
        }
      };
      postBody['ranking_details'].select_type = 'all';
    }
    this.rankingDetailService
      .delDetails(this.optimizationId, postBody)
      .subscribe(
        (result: any) => {
  ...
  startRankingDetails() {
    ...
    if (
      ...
      postBody['ranking_details'].details = this.rows;
      postBody['ranking_details'].select_type = 'current';
    } else if (this.selected.length === 0) {
      postBody['ranking_details'].sheets_setting = {
        table_setting: {
    ...
    this.rankingDetailService
      .startRanking(this.optimizationId, postBody)
      .subscribe(
        (result: any) => {
          if (result.status_code === 200) {
            this._message.success('竞价开启成功');
            this.refreshData();
          } else if (result.status_code === 303) {
            this.jumpToEdit('ranking_start', result.data.count);
          }
        },
        (err: any) => {
          this._message.error('开启失败,请重试');
        },
        () => { }
      );
  }
  ...
  stopRankingDetails() {
    ...
    const add_modal = this.modalService.create({
      nzTitle: '停止调价',
      nzWidth: 400,
      nzContent: PauseSettingComponent,
      nzClosable: false,
      nzMaskClosable: false,
      nzWrapClassName: 'pause-setting',
      nzFooter: null,
      nzComponentParams: {
        postBody: postBody,
    ...
    /*const postBody = {'ranking_details': {select_type: this.selectedType, details: [], sheets_setting: {}, type: 'keyword'}};
    if (this.selectedType === 'current' ||  this.pageInfo.allCount <= this.pageInfo.pageSize) {
      postBody['ranking_details'].details = this.selected;
      postBody['ranking_details'].select_type = 'current';
    } else {
      postBody['ranking_details'].sheets_setting = {table_setting: {...this.viewTableData, filter_optimization_ids: [this.optimizationId]}};
      postBody['ranking_details'].select_type = 'all';
    }

    this.rankingDetailService.stopRanking(this.optimizationId, postBody).subscribe((result: any) => {
        if (result.status_code === 200) {
  ...
  seeView(row) {
    ...
    if (row['html_status'] * 1 !== 2) {
      ...
      this.optimizationService.checkRankingHtml(parm).subscribe(result => {
        if (result['status_code'] === 200) {
          window.open(
