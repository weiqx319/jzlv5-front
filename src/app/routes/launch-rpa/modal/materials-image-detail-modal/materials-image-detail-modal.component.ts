...
export class MaterialsImageDetailModalComponent implements OnInit {
  ...
  getCoverMaterialDetail() {
    this.launchRpaService
      .getCoverMaterialImageDetail(this.materialId, {cid: this.cid})
      .subscribe(
        (results: any) => {
          if (results.status_code && results.status_code === 200) {
            this.material_make_time_str = results['data']['material_make_time'].substring(0, 10);
  ...
  doSave() {
    ...
    if (!this.saveing) {
      ...
      if(this.type === 'cover') {
        this.launchRpaService.updateCoverMaterialImageDetail(this.defaultData, {cid: this.cid}).subscribe(data => {
          this.saveing = false;
        ...
        this.launchRpaService.updateMaterialImageDetail(this.defaultData, {cid: this.cid}).subscribe(data => {
          this.saveing = false;
          if (data['status_code'] && data.status_code === 200) {
            this.message.success('保存成功');
            this.modalSubject.destroy('onOk');
