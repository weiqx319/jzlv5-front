...
export class UploadImageMaterialsComponent implements OnInit {
  ...
  getCoverImageSizeList() {
    this.launchRpaService.getCoverImageSize({publisher_id: this.menuService.currentPublisherId}).subscribe(result => {
  ...
  getImageSizeList() {
    let imageType;
    this.type === 'logo' ? imageType = 'logo' : imageType = 'default';
    this.launchRpaService.getImageSize({publisher_id: this.menuService.currentPublisherId}).subscribe(result => {
      if (result['status_code'] == 200) {
        if (isObject(result['data']) && result['data'].hasOwnProperty('image')) {
          if (this.cssType&&this.menuService.currentPublisherId==6) {
            result['data']['image'].forEach(item=> {
              if (this.imageTypeMap[this.cssType].indexOf(item.image_type)!==-1) {
                this.imageSizeList[item['width'] + 'x' + item['height']] = item;
                this.imageSizeShowList.push({
                  showSize: formatBytes(item.size),
                  ...item
  ...
  checkImageSize(width, height, size) {
    ...
    if (this.menuService.currentPublisherId == 7 || this.menuService.currentPublisherId == 16) {
  ...
  checkImage(fileInfo: IUploadImageFileInfo) {
    ...
    if (this.type === 'cover') {
      this.launchRpaService.checkCoverImageMd5(fileInfo.md5, {
        cid: this.cid,
      ...
      this.launchRpaService.checkImageMd5(fileInfo.md5, {
        cid: this.cid,
        publisher_id: this.menuService.currentPublisherId
      }).subscribe(result => {
        if (result['status_code'] == 200) {
  ...
  uploadImage(fileInfo: IUploadImageFileInfo) {
    ...
    if (this.type === 'cover') {
      this.launchRpaService.uploadCoverImageMaterials(formData, this.cid).subscribe(
        (event: HttpEvent<{}>) => {
      ...
      this.launchRpaService.uploadImageMaterials(formData, this.cid).subscribe(
        (event: HttpEvent<{}>) => {
  ...
  doSave() {
    ...
    if (postData.data_list.length > 0) {
      if (this.type === 'cover') {
        this.launchRpaService.saveCoverImageInfo(postData, {cid: this.cid}).subscribe(result => {
          if (result['status_code'] == 200) {
        ...
        this.launchRpaService.saveImageInfo(postData, {cid: this.cid}).subscribe(result => {
          if (result['status_code'] == 200) {
            this.message.success('保存成功');
            this.modalSubject.destroy('onOk');
            this.saving = false;
