...
@Component({
  selector: 'app-launch-group-template-qianchuan',
  templateUrl: './launch-group-template-qianchuan.component.html',
  styleUrls: ['./launch-group-template-qianchuan.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class LaunchGroupTemplateQianchuanComponent implements OnInit {
  ...
  public anchorList = [
    ...
    },
    {
      ...
      name: '广告组',
      sub: [
        { key: '#marketing_objective', name: '营销目标' },
        { key: '#promotion_objective', name: '推广方式' },
        { key: '#budget', name: '预算' },
        ...
        (results: any) => {
          if (results.status_code !== 200) {
            ...
            if (this.defaultData.template_struct.campaign.promotion_way === 'SIMPLE') {
              this.curBasicTarget = deepCopy(this.basicTargetList.filter(item => item.promotion_way === 'SIMPLE'));
            } else {
              this.curBasicTarget = deepCopy(this.basicTargetList.filter(item => item.promotion_way === 'STANDARD'));
            }
          }
        },
        (err: any) => {
      ...
      (results: any) => {
        if (results.status_code === 200) {
          results['data'].forEach(item => {
            item.name = item.aweme_name + '-' + JSON.parse(item.bind_type).join('-');
          });
  ...
  changeMarketingGoal(value) {
    this.defaultData.template_struct.campaign.marketing_goal = value;
    ...
    this.defaultData.template_struct.campaign.promotion_way = 'STANDARD';
    if (value === 'LIVE_PROM_GOODS') {
      this.popconObj.promotion_way = 'STANDARD';
      this.defaultData.template_struct.campaign.promotion_way = 'STANDARD';
      Object.keys(this.defaultData.template_struct.adgroup.product_aweme_id_lst).forEach(key => {
        this.defaultData.template_struct.adgroup.product_aweme_id_lst[key].product_id = [];
      });
      this.defaultData.template_struct.adgroup.product_info = [];
    ...
    if (this.defaultData.template_struct.campaign.promotion_way === 'SIMPLE') {
      ...
      // this.defaultData.template_struct.campaign.campaign_budget_mode=value==='VIDEO_PROM_GOODS'?'BUDGET_MODE_DAY':'BUDGET_MODE_TOTAL';
  ...
  changeLandType(value) {
    this.defaultData.template_struct.campaign.promotion_way = value;
    this.getCreativeConfigKey();
    if (value === 'SIMPLE') {
    ...
    this.curBasicTarget = this.basicTargetList.filter(item => item.promotion_way === value);
  ...
  changeAccount(type) {
    if (!type) {
      this.curAccountList = [];
      this.defaultData.chan_pub_id_lst[0] = this.chanPubId;
      this.defaultData.chan_pub_id_lst.forEach(id => {
        const data = this.accountsList.find(item => item.chan_pub_id == this.defaultData.chan_pub_id_lst);
        if (data) {
          this.defaultData.template_struct.adgroup.product_aweme_id_lst[id] = {
            aweme_id: null,
            product_id: [],
          };
          this.curAccountList.push({
  ...
  changeAweme(value, list) {
    ...
    if (awemeData) {
      this.defaultData.template_struct.creative.bind_type = JSON.parse(awemeData.bind_type);
      if (this.defaultData.template_struct.creative.bind_type.indexOf('OFFICIAL') === -1 && this.defaultData.template_struct.creative.bind_type.indexOf('SELF') === -1) {
        this.defaultData.template_struct.creative.creative_material_mode = 'CUSTOM_CREATIVE';
        this.defaultData.template_struct.creative.is_homepage_hide = 0;
        this.popconObj.creative_auto_generate = '0';
