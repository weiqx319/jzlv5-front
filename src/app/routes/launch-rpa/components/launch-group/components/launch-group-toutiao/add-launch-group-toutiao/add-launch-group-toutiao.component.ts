...
@Component({
  selector: 'app-add-launch-group-toutiao',
  templateUrl: './add-launch-group-toutiao.component.html',
  styleUrls: ['./add-launch-group-toutiao.component.scss']
})
export class AddLaunchGroupToutiaoComponent implements OnInit {
  ...
  getFeedConfigCreativeCategory() {
    this.launchRpaService.getFeedConfigCreativeCategory()
      .subscribe(
        (results: any) => {
  ...
  doSave() {
    ...
    if (!this.submit) {
      ...
      if(this.isEdit) {
        this.launchRpaService.updateLaunchProjectl(this.launchGroupId, body, {cid: this.authService.getCurrentUserOperdInfo().select_cid,}).subscribe(result => {
          this.submit = false;
        ...
        this.launchRpaService.createLaunchProject(body, {cid: this.authService.getCurrentUserOperdInfo().select_cid,}).subscribe(result => {
          this.submit = false;
          if (result.status_code && result.status_code === 200) {
