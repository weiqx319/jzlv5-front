...
@Component({
  selector: 'app-target-basic-template-qc',
  templateUrl: './target-basic-template-qc.component.html',
  styleUrls: ['./target-basic-template-qc.component.scss'],
  encapsulation:ViewEncapsulation.None
})
export class TargetBasicTemplateQcComponent implements OnInit,AfterViewChecked,OnChanges {
  ...
  public awemeBehaviorList=[
    {title:'关注',key:'FOLLOWED_USER',isLeaf: true},
    {title:'视频互动',key:'COMMENTED_USER-LIKED_USER-SHARED_USER',children: [
        {title:'评论',key:'COMMENTED_USER',isLeaf: true},
        {title:'点赞',key:'LIKED_USER',isLeaf: true},
        {title:'分享',key:'SHARED_USER',isLeaf: true},
      ]},
    {title:'直播互动',key:'LIVE_WATCH-LIVE_EFFECTIVE_WATCH-LIVE_COMMENT-LIVE_EXCEPTIONAL-LIVE_GOODS_CLICK-LIVE_GOODS_ORDER',children: [
        {title:'观看',key:'LIVE_WATCH',isLeaf: true},
        {title:'有效观看',key:'LIVE_EFFECTIVE_WATCH',isLeaf: true},
        {title:'直播评论',key:'LIVE_COMMENT',isLeaf: true},
        {title:'打赏',key:'LIVE_EXCEPTIONAL',isLeaf: true},
        {title:'商品点击',key:'LIVE_GOODS_CLICK',isLeaf: true},
        {title:'商品下单',key:'LIVE_GOODS_ORDER',isLeaf: true},
      ]},
    {title:'商品互动',key:'GOODS_CARTS_CLICK-GOODS_CARTS_ORDER',children: [
        {title:'购物车点击',key:'GOODS_CARTS_CLICK',isLeaf: true},
        {title:'购物车下单',key:'GOODS_CARTS_ORDER',isLeaf: true},
      ]},
  ];
  ...
  public editDefaultDataOption = {
    ...
    aweme_fan_categories: {
      ...
      aweme_fan_behaviors_days: {
        resultValue: 'DAYS_15',
        sub: [
          {label:'近15天',value:'DAYS_15'},
          {label:'近30天',value:'DAYS_30'},
          {label:'近60天',value:'DAYS_60'},
        ],
      },
      aweme_fan_behaviors: {
        resultList: ['FOLLOWED_USER'],
        sub: [],
      },
    },
    platform: {
  ...
  initSingleData(audienceData) {
    if (Object.keys(audienceData).length) {
      // 人群
      this.editDefaultDataOption.audience_mode['resultValue'] = audienceData['audience_mode']||'NONE';
      ...
      if (audienceData['aweme_fan_categories'] !== undefined) {
        if (isArray(audienceData['aweme_fan_categories']) && audienceData['aweme_fan_categories'].length > 0) {
          ...
          this.editDefaultDataOption.aweme_fan_categories.aweme_fan_behaviors_days['resultValue'] = audienceData['aweme_fan_categories']['aweme_fan_behaviors_days']||'DAYS_15';
        ...
        if (audienceData['aweme_fan_behaviors']) {
          this.editDefaultDataOption.aweme_fan_categories['aweme_fan_behaviors']['resultList'] = [...audienceData['aweme_fan_behaviors']];
        }
      }
  ...
  landingTypeChange() {
    ...
    if (this.promotion_way==='STANDARD') {
      if (this.editDefaultDataOption.audience_mode.resultValue=='AUTO') {
        this.editDefaultDataOption.audience_mode.resultValue='CUSTOM';
      }
      this.editDefaultDataOption['audience_mode'].sub.forEach(item => {
        item['disabled'] = false;
      ...
      this.accounts=[];
      this.curAccountsList=[];
      if (this.editDefaultDataOption.audience_mode.resultValue=='NONE') {
        this.editDefaultDataOption.audience_mode.resultValue='CUSTOM';
      }
      this.editDefaultDataOption['audience_mode'].sub.forEach(item => {
        item['disabled'] = false;
  ...
  generateResultData() {
    ...
    if (1||this.editDefaultDataOption.audience_mode['resultValue']!=='AUTO') {
      ...
      resultData['aweme_fan_categories'] = [];
      resultData['aweme_fan_categories_slt'] = this.editDefaultDataOption.aweme_fan_categories['resultValue'];
      if (this.editDefaultDataOption.aweme_fan_categories['resultValue'] !== 'NONE') {
        ...
        this.editDefaultDataOption.aweme_fan_categories['aweme_fan_behaviors']['resultList'].map(item=> {
          resultData['aweme_fan_behaviors']=[...resultData['aweme_fan_behaviors'],...item.split('-')];
        });
        resultData['aweme_fan_behaviors_days']=this.editDefaultDataOption.aweme_fan_categories['aweme_fan_behaviors_days']['resultValue'];
      }
    }
