...
@Component({
  selector: 'app-creative-template-bd',
  templateUrl: './creative-template-bd.component.html',
  styleUrls: ['./creative-template-bd.component.scss']
})
export class CreativeTemplateBdComponent implements OnInit {
  @Input() defaultData;
  @Input() resultData;
  @Input() checkErrorTip;
  ...
  openMaterials(data,tempData,isBatch?: boolean) {
    if (!data) {
      data=this.defaultData.template_struct.material.material_type_lst['all'].materials[0].limit[0];
  ...
  addMaterials(data: any[],source,isBatch) {
    ...
    const maxSelect=maxNum*this.defaultData.template_struct.material.material_type_lst['all'].materials.length.toFixed(0);
    ...
    add_modal.afterClose.subscribe(result => {
      if (isObject(result) && result.result == 'ok') {
        ...
        if (isBatch) {
          this.batchAssignMaterial(0,maxNum,result['data'],this.defaultData.template_struct.material.material_type_lst['all'].materials,0,source.css_type);
        } else {
  ...
  addImageMaterials(data: any[], source,isBatch) {
    let tempData = [];
    const maxNum = this.cssTypeObj[source.css_type] == 2?this.defaultData.template_struct.material.material_type_temp.num*3:this.defaultData.template_struct.material.material_type_temp.num;
    ...
    add_modal.afterClose.subscribe(result => {
      if (isObject(result) && result.result == 'ok') {
        if (this.cssTypeObj[source.css_type] !== 2) {
          ...
          if (isBatch) {
            this.batchAssignMaterial(0,this.defaultData.template_struct.material.material_type_temp.num,result['data'],this.defaultData.template_struct.material.material_type_lst['all'].materials,0,source.css_type);
          } else {
            data.splice(0, data.length, ...result['data']);
          }
          ...
          for (let i = 0; i < [...result['data']].length; i += 3) {
            resultData.push({ materials: [...result['data']].slice(i, i + 3), title: [],sub_title:[] });
          }
          if (isBatch) {
            this.batchAssignMaterial(0,this.defaultData.template_struct.material.material_type_temp.num,resultData,this.defaultData.template_struct.material.material_type_lst['all'].materials,0,source.css_type);
          } else {
  ...
  batchAddTitle(type,maxNum) {
    ...
    add_modal.afterClose.subscribe(result => {
      if (isObject(result) && result.result == 'ok') {
        this.batchAssignTitle(0,maxNum,result['data'],this.defaultData.template_struct.material.material_type_lst['all'].materials,0,type);
      }
    });
  }
