...
@Component({
  selector: 'app-launch-target-list-gdt',
  templateUrl: './launch-target-list-gdt.component.html',
  styleUrls: ['./launch-target-list-gdt.component.scss']
})
export class LaunchTargetListGdtComponent implements OnInit {
  public apiData = [];
  public total = 0;
  public loading = true;
  ...
  getChildTargetList(event,value) {
    ...
    if(event) {
      this.launchRpaService
        .getTargetListByGdt(value.audience_template_id,{
          cid: this.cid,
        })
        .subscribe(
          (results: any) => {
  ...
  deleteTargetBasicTemplate(id) {
    const ids = [id];
    this.launchRpaService
      .deleteTargetBasicTemplate({id_list: ids}, {
        cid: this.cid,
      })
      .subscribe(
        (result: any) => {
