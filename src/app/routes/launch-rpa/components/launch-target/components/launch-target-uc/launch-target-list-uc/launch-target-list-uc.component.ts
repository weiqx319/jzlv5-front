...
@Component({
  selector: 'app-launch-target-list-uc',
  templateUrl: './launch-target-list-uc.component.html',
  styleUrls: ['./launch-target-list-uc.component.scss']
})
export class LaunchTargetListUcComponent implements OnInit {
  public apiData = [];
  public total = 0;
  public loading = true;
  ...
  getChildTargetList(event,value) {
    ...
    if(event) {
      this.launchRpaService
        .getTargetListByUc(value.audience_template_id,{
          cid: this.cid,
        })
        .subscribe(
          (results: any) => {
  ...
  deleteTargetBasicTemplate(id) {
    const ids = [id];
    this.launchRpaService
      .deleteTargetBasicTemplateUc({id_list: ids}, {
        cid: this.cid,
      })
      .subscribe(
        (result: any) => {
