...
@Component({
  selector: 'app-launch-target-list-niu',
  templateUrl: './launch-target-list-niu.component.html',
  styleUrls: ['./launch-target-list-niu.component.scss']
})
export class LaunchTargetListNiuComponent implements OnInit {
  ...
  getChildTargetList(event, value) {
    ...
    if (event) {
      this.launchRpaService
        .getTargetListByNiu(value.audience_template_id, {
          cid: this.cid,
        })
        .subscribe(
          (results: any) => {
  ...
  deleteTargetBasicTemplate(id) {
    const ids = [id];
    this.launchRpaService
      .deleteTargetBasicTemplateNiu({ id_list: ids }, {
        cid: this.cid,
      })
      .subscribe(
        (result: any) => {
