...
export class DataViewEditWrapService {
  ...
  editAccount(publisher_id, data, edit_type, row?): Observable<boolean>  {
    return this.dataViewService.editAccount( publisher_id , data, edit_type).pipe(map(
      (result: any) => {
        if (row) {
          row['saveing'] = false;
  ...
  editKeyword(publisher_id, data, edit_type, row?): Observable<boolean> {
    if (row) {
      row['saveing'] = true;
    }
    const notifyData: any[] = [];
    const userOperdInfo = this.authService.getCurrentUserOperdInfo();
    return this.dataViewService.editKeyword(publisher_id, data, edit_type).pipe(map(
      (result: any) => {
        if (row) {
          row['saveing'] = false;
          row['showPriceBtn'] = false;
