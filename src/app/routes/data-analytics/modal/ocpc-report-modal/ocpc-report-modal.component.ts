...
// 其他文件
import { ViewBatchUploadComponent } from "../../../data-view/modal/view-batch-upload/view-batch-upload.component";
import { DataViewOptimizationService } from '../../../data-view/service/data-view-optimization.service';
import { DataViewFolderService } from '../../../data-view/service/data-view-folder.service';
import { ViewBatchUploadDeleteComponent } from '../../../data-view/modal/view-batch-upload-delete/view-batch-upload-delete.component';
import { FengwuPreviewComponent } from "../../../data-view/modal/fengwu-preview/fengwu-preview.component";
...
@Component({
  selector: 'app-ocpc-report-modal',
  templateUrl: './ocpc-report-modal.component.html',
  styleUrls: ['../../../../../styles/routes/routes.scss', './ocpc-report-modal.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [TableItemService, DataViewOptimizationService, DataViewFolderService],
})
export class OcpcReportModalComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.summary_type_name = this.dataAnalyticsService.summary_type_name;
  ...
  ngOnInit() {
    ...
    this.viewTableData['locked_items'] = this.viewItemService.getLockedItemsBySummaryType(this.source_summary, true);
    ...
    if (localMarkInfo === null) {
      ...
      if (this.source_summary !== 'ocpc_360' && this.source_summary !== 'ocpc_baidu' && this.source_summary !== 'creative_fengwu_360') {
  ...
  getNegativeWordGroups() {
    this.dataAnalyticsService.getAllNegativeWordGroupList().subscribe(
      (result) => {
  ...
  getAccountPublishers() {
    this.dataAnalyticsService.getAccountPublishers().subscribe(
      (result) => {
      ...
      if (this.source_summary === 'ocpc_baidu') {
        ...
        if (this.selected.length === 1) {
          ...
          if (this.quickEditParam.result['result']['value']) {
            ...
            if (this.quickEditParam.publisherId === 3) {
  ...
  doSync(chan_pub_id?) {
    ...
    if (this.viewTableData.summary_type === 'account') {
      ...
      if (postBody.info.length > 0) {
        this.dataAnalyticsService.syncPublisher(postBody).subscribe((result: any) => {
          ...
          if (results.status_code && results.status_code === 200) {
            ...
            this.rows.map((item, index) => {
              ...
              if (item.publisher_id) {
                item['matchTypeList'] = this.dataAnalyticsService.matchTypes[item.publisher_id];
              }
            });
  ...
  refreshFields(data: any[]) {
    ...
    const operateColumn: any = [{ width: '100', name: '操作', prop: 'operateColumn', operate: true, frozenLeft: true, sortable: false }];
    ...
    if (this.endLocked_items.length && (this.userSelectedOper.role_id !== 5 && this.userSelectedOper.role_id !== 2 && this.userSelectedOper.role_id !== 6)) {
      if (this.source_summary != 'dsa_pattern_day' && this.source_summary != 'ocpc_360' && this.source_summary != 'ocpc_baidu' && this.source_summary != 'ocpc_baidu_report' && this.source_summary != 'creative_fengwu_360') {
        this.columns.push(...lockedColumn, ...dataAnalysis, ...tmpFiled, ...endLockedColumn);
      } else if (this.source_summary == 'ocpc_baidu') {
  ...
  startGetRankingCode(setting, row, index, originObserver) {
    ...
    this.dataAnalyticsService.getRankingCode({ detail_info: detailInfo }, queryParams).subscribe((resultData) => {
    ...
    this.accountPublishers.forEach((item) => {
      ...
      if (item.publisher_id * 1 === itemData.publisher_id * 1) {
        ...
        switch (item.publisher_id * 1) {
          case 1:
            this.matchTypeData = this.dataAnalyticsService.matchType_baidu;
            break;
          case 3:
            this.matchTypeData = this.dataAnalyticsService.Match_type_360;
            break;
          case 4:
            this.matchTypeData = this.dataAnalyticsService.match_type_shenma;
            break;
          case 2:
            this.matchTypeData = this.dataAnalyticsService.match_type_sougou;
            break;
        }
      }
    });
  ...
  clickKeyOk(row) {
    ...
    this.dataAnalyticsService.addSingleKeyword(this.addWordData).subscribe(
      (result: any) => {
        if (result.status_code === 200) {
  ...
  clickNagativeOk(row) {
    ...
    if (this.addNegativeWordGroupData.is_add === 1) {
      ...
      this.dataAnalyticsService.createNegativeWord(postBody).subscribe(data => {
  ...
  editCreative(data, edit_type, row?) {
    ...
    this.dataAnalyticsService.editCreative(this.publisherId, data, edit_type).subscribe(
      (result: any) => {
  ...
  editKeyword(data, edit_type, row?) {
    ...
    const userOperdInfo = this.authService.getCurrentUserOperdInfo();
    this.quickEditParam.saveing = true;
    this.dataAnalyticsService.editKeyword(this.publisherId, data, edit_type).subscribe(
      (result: any) => {
        this.quickEditParam.saveing = false;
        if (row) {
          row['saveing'] = false;
  ...
  editCampaign(data, edit_type, row) {
    this.dataAnalyticsService.editCampaign(this.publisherId, data, edit_type).subscribe(
      (result: any) => {
        if (row) {
          row['saveing'] = false;
  ...
  editAdgroup(data, edit_type, row?) {
    ...
    this.dataAnalyticsService.editAdgroup(this.publisherId, data, edit_type).subscribe(
      (result: any) => {
        this.quickEditParam.saveing = false;
        if (row) {
          row['saveing'] = false;
  ...
  editAccount(data, edit_type, row?) {
    this.dataAnalyticsService.editAccount(this.publisherId, data, edit_type).subscribe(
      (result: any) => {
        if (row) {
          row['saveing'] = false;
          row['showBudgetBtn'] = false;
  ...
  editOcpcBaidu(data, edit_type, row?) {
    this.dataAnalyticsService.editOcpcBaidu(data, edit_type).subscribe(
      (result: any) => {
        if (row) {
          row['saveing'] = false;
  ...
  seeView(row) {
    ...
    if (row['html_status'] * 1 !== 2) {
      ...
      this.dataAnalyticsService.checkRankingHtml(parm).subscribe((result) => {
        if (result['status_code'] === 200) {
  ...
  // 打开生成报表
  openReportTpl() {
    const menuDetail = this.dataAnalyticsService.menuDetail[this.source_summary];
    if (menuDetail) {
