...
@Component({
  selector: 'app-default-channel',
  templateUrl: './default-channel.component.html',
  styleUrls: ['./default-channel.component.scss']
})
export class DefaultChannelComponent implements OnInit {
  ...
  refreshData(status?) {
    ...
    this.defineSettingService.getDefaultChannelList({ page: this.currentPage, count: this.pageSize }).subscribe(
      (results: any) => {
  ...
  downloadList() {
    window.open(environment.SERVER_API_URL + '/setting/channel/download');
  }
}
