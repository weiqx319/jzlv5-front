...
export class ReportTableShowComponent implements OnInit {
  ...
  @Input() set tableSetting(value: any) {
    ...
    if (!isUndefined(value) && !isUndefined(value.selected_items) && !isUndefined(value.locked_items)) {
      this.refreshFields(value.selected_items, value.locked_items, value.report_type, value.time_grain);
  ...
  refreshFields(data: any[], lockedData: any[], reportType, timeGrain) {
    ...
    if (reportType === 'hours_report') {
      this.tableHeads.push(...hourFiledColumn);
    }
    if (timeGrain === 'day') {
      this.tableHeads.push(...timeFiledColumn);
    }

    this.tableHeads.push(...lockedColumn, ...tmpFiled);
