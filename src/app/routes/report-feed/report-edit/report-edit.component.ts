...
import { TableStepFeedComponent } from "../../../module/table-setting/components/table-step/table-step-feed.component";
...
export class ReportEditComponent implements OnInit {
  ...
  constructor(private modalService: NzModalService,
    ...
    this.reportSetting.channel_id = this.route.snapshot.queryParams['channel_id'];
    ...
    if (this.reportId && this.reportId.length > 0) {
      ...
      this.reportService.getReportTplDetail(this.tplId).subscribe((data) => {
        if (data.status_code === 200) {
          ...
          this.reportSetting.report_freq = reportResult.report_freq;
          this.reportSetting.sheets_setting = reportResult.sheets_setting;
          this.removeUseless(this.reportSetting.sheets_setting);
  ...
  //移除没用的item
  removeUseless(tableSetting) {
    tableSetting.forEach(item => {
      const newItem = this.tableItemService.getTableItems(item['table_setting']['report_type'], item['table_setting']['summary_type'], JSON.parse(JSON.stringify(item['table_setting']['selected_items'])));
      item['table_setting']['selected_items'] = newItem['selected'];
    });
  }
  ...
  addReportTable(): any {
    ...
    const add_modal = this.modalService.create({
      nzTitle: '添加表格',
      nzWidth: 1000,
      nzContent: TableStepFeedComponent,
      nzClosable: false,
    ...
    if (this.reportId && this.reportId.length > 0) {
      ...
      this.reportService.updateReport(putBody).subscribe((data: any) => {
        if (data.status_code === 200) {
          ...
          this.router.navigateByUrl('/report/feed/list');
      ...
      this.reportService.createReport(postBody).subscribe((data: any) => {
        if (data.status_code === 200) {
          ...
          // this.router.navigate(['/report/feed/list']);
          this.router.navigateByUrl('/report/feed/list');
