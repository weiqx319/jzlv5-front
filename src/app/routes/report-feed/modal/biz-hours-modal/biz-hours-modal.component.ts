...
@Component({
  selector: 'app-biz-hours-modal',
  templateUrl: './biz-hours-modal.component.html',
  styleUrls: ['./biz-hours-modal.component.scss'],
  providers: [TableItemFeedService]
})
export class BizHoursModalComponent extends DataViewTableComponent implements OnInit {
  ...
  constructor(private modalService: NzModalService,
    ...
    this.viewTableData['summary_type'] = "biz_unit_account_hours";
    this.viewTableData['sub_summary_type'] = "biz_unit_account_hours";
    this.source_summary = "biz_unit_account_hours";
    this.viewTableData['report_type'] = "biz_unit_hours_report";
  }

  ngOnInit() {
    this.getBizList();
    this.viewReportSetting.report_name = '定制报表-' + '小时报表';
  ...
  refreshFields(data: any[]) {
    ...
    this.viewTableData['locked_items'].forEach(item => {
      ...
      if (result.isHas || item['key'] === 'hours') {
        if (result.isHas) {
  ...
  generateHoursShow() {
    ...
    if (originHours.length) {
      originHours.forEach(item => {
        const startHour = item['startHour'] + ":00:00";
