...
export class JzlChartService {
  ...
  list(): Observable<any[]> {
    return this._httpClient.get("/dashboard?channel_id=").pipe(map((response) => {
  ...
  getDashBoard(dashboard_id): Observable<any>  {
    const url =  '/dashboard/chart/' + dashboard_id + '/list?channel_id=';
    return this._httpClient.get(url).pipe(map((response) => {
      return response.data;
    }));
  }

  getChart(company,dashboard_id, chart_id,dashboard_config?): Observable<any> {
    // const url = '/chart/' + chart_id;
    // const url = '/dashboard/chart/' + chart_id;
    let  url = '/dashboard/chart/data?';
    if(company) {
      url = '/company/dashboard/chart/data?';
    }

    // const url = 'http://rap2api.taobao.org/app/mock/2017/GET/chartdetail';
    if(dashboard_config!=undefined) {
      return this._httpClient.post(url, { data_source: 'id', dashboard_id, chart_id, dashboard_config});
    } else {
      return this._httpClient.post(url, { data_source: 'id', dashboard_id, chart_id});
    }
